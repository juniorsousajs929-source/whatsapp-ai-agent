...
async function testRotator() {
    console.log("🔄 TESTING LIVE ROTATOR (Check Distribution)...");
    const counts = {};
    for (let i = 0; i < 10; i++) {
        try {
...
async function testWebhook(botId) {
    console.log(`\n🤖 TESTING LIVE WEBHOOK (${botId})...`);
    try {
        const payload = {
            user_id: `test_live_user_${botId}_${Date.now()}`,
...
async function run() {
    await testRotator();
    await testWebhook('zap5'); // Should work
    await testWebhook('zap6'); // Should work now with new token
}
